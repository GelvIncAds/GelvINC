...
export const AdminPortal: React.FC<AdminPortalProps> = ({
  ...
  const handleQuickFullCapacity = async (item: AdItem) => {
    const target = item.totalCapacity || Math.max(item.stock + 10, 10);
    setSavingItemIds((prev) => ({ ...prev, [item.id]: true }));
    try {
      await onUpdateStock(item.id, target, `Recalibrated to full warehouse capacity (${target} units)`);
      await fetchAuditLogs();
      showToast(`'${item.title}' replenished to Full Capacity (${target} units)`);
    } catch (err: any) {
      alert("Failed to update stock: " + err.message);
    } finally {
      setSavingItemIds((prev) => ({ ...prev, [item.id]: false }));
  ...
  const handleAddItemSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onAddItem({
        ...
        totalCapacity: Math.max(Number(newStock), 50),
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-6">
          ...
          <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 bg-slate-50 p-3.5 rounded-2xl border border-slate-200 text-xs">
            ...
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input
                type="text"
                placeholder="Search material, location, city, category..."
            ...
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="border-b border-slate-200 text-slate-500 font-bold uppercase tracking-wider bg-slate-50/80">
                  ...
                  <th className="p-3.5 min-w-[200px]">Live Stock &amp; Capacity</th>
                ...
                {items.length === 0 ? (
                  ...
                  .filter((item) => {
                    const matchesSearch =
                      ...
                      item.city.toLowerCase().includes(stockSearchQuery.toLowerCase()) ||
                  ...
                  .map((item) => {
                    ...
                    const percentOfCap = Math.min(100, Math.round((item.stock / (item.totalCapacity || 10)) * 100));
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        {/* 4. Stock & Capacity Bar */}
                        <td className="p-3.5">
                          <div className="space-y-1.5">
                            <div className="flex items-center justify-between text-xs">
                              <div className="flex items-center space-x-1.5">
                                <span className="font-black text-sm text-slate-900 font-mono">{item.stock}</span>
                                <span className="text-slate-400">/ {item.totalCapacity} Max</span>
                              </div>
                        ...
                        {req.items.map((item, idx) => {
                          ...
                          return (
                            <div key={idx} className="p-3 bg-slate-50 rounded-2xl border border-slate-200/80 flex justify-between items-center">
                              ...
                              <div className="bg-amber-100/70 text-amber-900 border border-amber-200 px-3 py-1.5 rounded-xl font-extrabold text-xs">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-[11px] text-slate-500 font-medium truncate">
                    {calibratingItem.city} &bull; {calibratingItem.location}
                  </span>
                </div>
                ...
                <div className="flex items-center space-x-3 text-xs text-slate-600 mt-1">
                  ...
                  <span>&bull;</span>
                  <span>Capacity: <strong className="text-slate-900">{calibratingItem.totalCapacity} Max</strong></span>
                </div>
              </div>
            </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <div className="flex flex-wrap gap-1.5 flex-1">
                        ...
                        <button
                          type="button"
                          onClick={() => setRecalibrateValue(Math.round((calibratingItem.totalCapacity || 10) * 0.5))}
                        ...
                        >
                          50% Cap ({Math.round((calibratingItem.totalCapacity || 10) * 0.5)})
                        </button>
                        <button
                          type="button"
                          onClick={() => setRecalibrateValue(calibratingItem.totalCapacity || 10)}
                          className="px-2.5 py-1.5 rounded-lg font-bold text-xs bg-emerald-50 text-emerald-800 border border-emerald-200 hover:bg-emerald-100"
                        >
                          Max Cap ({calibratingItem.totalCapacity || 10})
                        </button>
                      </div>
                    </div>
                  </div>
